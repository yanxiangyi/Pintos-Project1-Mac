...
static bool priority_compare(const struct list_elem *e1, const struct list_elem *e2);
...
thread_tick (void) 
{
  ...
  /* Enforce preemption. */
  if (++thread_ticks >= thread_current()->slices) {
      intr_yield_on_return();
      t->priority=t->priority-3>0?t->priority-3:0;
...
static bool
priority_compare(const struct list_elem *e1, const struct list_elem *e2){
    if(list_entry(e1, struct thread, elem)->priority > list_entry(e2, struct thread, elem)->priority)
        return true;
    else
        return false;
}
