...
////        printf ("(%c%c,%d,%d) ", t->name[len - 2], t->name[len - 1], t->priority, t->base_priority);
...
//    //things to help us testing your program  ***   !!!
    ...
    if (++thread_ticks >= thread_current()->slices) {
        ...
        if (strcmp(t->name,"main") != 0) {
            ...
            t->base_priority = t->base_priority - 3 > 0 ? t->base_priority - 3 : 0;
        }
    }
}
...
void
thread_sleep_remaining (struct thread *t, void *aux UNUSED) {
...
void
thread_reinsert (struct thread *t) {
    list_remove(&t->elem);
    list_insert_ordered(&ready_list, &t->elem, thread_compare, NULL);
